...
export function Awards({
  ...
  // Points for circular rotation in stamp variant
  const points = [
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setRotation((prev) => prev + 72) // 360 / 5 = 72 degrees per point
  ...
  if (variant === "stamp") {
    ...
    // Calculate positions for circular text
    const centerX = 96
    const centerY = 96
    const textRadius = 65
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* Rotating circular text points */}
          {points.map((point, index) => {
            const baseAngle = (index * 72) * (Math.PI / 180) // 360/5 = 72 degrees
            ...
            return (
              <text
                ...
                className={cn(
                  "text-[7px] font-semibold transition-all duration-700",
